...
import { useAuth, DEFAULT_EMAIL, DEFAULT_PASS } from '@/hooks/useAuth';

/* ── Explicit tokens — never rely on CSS vars that flip on dark themes ── */
const DARK   = '#09080A';        /* near-black warm */
const PANEL  = '#0F0D10';        /* left panel bg   */
const W      = '#FFFFFF';
const CREAM  = '#FAF9F7';
const AC     = '#9D7E3F';        /* gold            */
const ACL    = '#C4A76B';        /* gold light      */
const MU     = '#6B7280';
const BORDER = '#E5E1DA';
...
export default function Auth() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      const { ok, error: err } = signIn(email, password);
      setLoading(false);
        ...
        <div className="w-full" style={{ maxWidth: '400px' }}>
          ...
          <div className="mt-5" style={{ border: `1px solid ${BORDER}` }}>
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 text-[9px] uppercase tracking-[0.24em] font-bold transition-colors"
              style={{ color: MU }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = DARK; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = MU; }}>
              <span>Demo Credentials</span>
              <motion.span animate={{ rotate: hintOpen ? 45 : 0 }} transition={{ duration: 0.18 }} className="text-[14px] font-light">+</motion.span>
            </button>
            <AnimatePresence>
              {hintOpen && (
                <motion.div
                  ...
                  style={{ borderTop: `1px solid ${BORDER}`, backgroundColor: W }}>
                  <div className="px-4 py-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] uppercase tracking-[0.2em] font-bold" style={{ color: MU }}>Email</span>
                      <button type="button" onClick={() => setEmail(DEFAULT_EMAIL)}
                        className="text-[12px] font-mono transition-colors" style={{ color: DARK }}
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] uppercase tracking-[0.2em] font-bold" style={{ color: MU }}>Password</span>
                      <button type="button" onClick={() => setPassword(DEFAULT_PASS)}
                        className="text-[12px] font-mono transition-colors" style={{ color: DARK }}
                        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = AC; }}
                        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = DARK; }}>
                        {DEFAULT_PASS}
                    ...
                    <p className="text-[9px] font-light" style={{ color: MU }}>
                      Click email or password to auto-fill. Change credentials in Settings after signing in.
                    </p>
                  </div>
                </motion.div>
              )}
