...
export default function Ledger() {
  ...
  return (
    <AppShell>
      ...
      <div className="px-8 py-5 border-b border-border grid grid-cols-3 gap-px bg-border">
        {[{ l: 'Inflow', v: fmtUSD(totals.inflow), c: 'text-positive' }, { l: 'Outflow', v: fmtUSD(totals.outflow), c: '' }, { l: 'Net Position', v: fmtUSD(totals.net), c: totals.net >= 0 ? 'text-positive' : 'text-accent' }].map(s => (
          <div key={s.l} className="bg-background px-5 py-3"><div className="micro-label">{s.l}</div><div className={`text-xl font-semibold font-mono-tab mt-1 ${s.c}`}>{s.v}</div></div>
        ))}
      </div>

      <div className="px-8 py-5 border-b border-border flex gap-3 items-center flex-wrap">
        <Input placeholder="Search reference or counterparty…" value={q} onChange={e => setQ(e.target.value)} className="rounded-none max-w-sm h-9" />
        <Select value={type} onValueChange={setType}>
          <SelectTrigger className="rounded-none w-36 h-9"><SelectValue /></SelectTrigger>
        ...
        <Select value={project} onValueChange={setProject}>
          <SelectTrigger className="rounded-none w-52 h-9"><SelectValue /></SelectTrigger>
        ...
        <div className="ml-auto text-xs text-muted-foreground font-mono-tab">{rows.length} entries</div>
      </div>
