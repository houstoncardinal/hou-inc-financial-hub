  ...
  return (
    <PortalLayout>
      <motion.div variants={stagger} initial="hidden" animate="show">
        ...
        {/* ══════════════════════════════════════
            HERO — light, matches admin/dashboard header language
        ══════════════════════════════════════ */}
        <motion.div variants={fade} className="px-5 sm:px-8 md:px-10 pt-8 sm:pt-10 pb-6" style={{ borderBottom: `1px solid ${BORDER}` }}>
          <div className="flex items-center justify-between gap-6">
            <div className="flex-1 min-w-0">
              {/* Eyebrow + Live badge */}
              <div className="flex items-center gap-3 mb-3">
                <div className="text-[9px] uppercase tracking-[0.4em] font-bold" style={{ color: MUTED }}>
                  Project Schedule
                </div>
                <AnimatePresence>
                  {dataLoaded && (
                    <motion.div
                      ...
                      style={{
                        backgroundColor: liveFlash ? 'rgba(16,185,129,0.15)' : 'rgba(16,185,129,0.08)',
                        border: `1px solid ${liveFlash ? 'rgba(16,185,129,0.4)' : 'rgba(16,185,129,0.18)'}`,
              ...
              <div style={{
                ...
                fontSize: 'clamp(28px, 5vw, 46px)', color: DARK,
              ...
              {/* Stats row */}
              <div className="flex items-center gap-0 mt-5 pt-4" style={{ borderTop: `1px solid ${BORDER}` }}>
                {[
                  { n: completedCount, label: 'Complete',  color: GREEN },
                  { n: activeRow ? 1 : 0, label: 'Active', color: ACCENT },
                  { n: Math.max(0, total - completedCount - (activeRow ? 1 : 0)), label: 'Upcoming', color: MUTED },
                ].map(({ n, label, color }, i) => (
                  <div key={label} className="flex items-center">
                    {i > 0 && <div className="w-px h-6 mx-4 sm:mx-5" style={{ backgroundColor: BORDER }} />}
                    <div>
                      <div style={{ fontFamily: SERIF, fontSize: 'clamp(20px, 3.6vw, 26px)', fontWeight: 700, color, lineHeight: 1 }}>{n}</div>
                      <div style={{ fontSize: 7, textTransform: 'uppercase', letterSpacing: '0.26em', color: MUTED, fontWeight: 600, marginTop: 3 }}>
                        {label}
                      </div>
                    </div>
                  </div>
        ...
        <div className="px-4 sm:px-6 md:px-10 py-6 sm:py-8 max-w-3xl">
          ...
          {dataLoaded && total === 0 && (
            <motion.div variants={fade}
              ...
              <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
                style={{ backgroundColor: 'rgba(0,0,0,0.07)', border: '1px solid rgba(0,0,0,0.2)' }}>
                <Clock className="w-4 h-4" style={{ color: ACCENT_SOFT }} strokeWidth={1.5} />
              </div>
              <div>
          ...
          {activeRow && (
            <motion.div variants={fade}
              ...
              style={{ backgroundColor: 'rgba(0,0,0,0.06)', border: `1px solid rgba(0,0,0,0.28)` }}>
              <div className="w-2.5 h-2.5 rounded-full animate-pulse shrink-0" style={{ backgroundColor: ACCENT }} />
              <div className="flex-1 min-w-0">
                <div className="text-[7px] uppercase tracking-[0.32em] font-bold mb-0.5" style={{ color: ACCENT }}>Currently In Progress</div>
              ...
              <ChevronRight className="w-4 h-4 shrink-0" style={{ color: 'rgba(0,0,0,0.4)' }} strokeWidth={2} />
            </motion.div>
          )}
          ...
          {total > 0 && (
            <motion.div variants={fade}
              className="mb-6 p-6"
              style={{ backgroundColor: WHITE, border: `1px solid ${BORDER}`, boxShadow: '0 1px 8px rgba(17,24,39,0.04)' }}>
              <div className="flex items-end justify-between mb-4">
                <div>
                  <div className="text-[9px] uppercase tracking-[0.36em] font-bold mb-1" style={{ color: ACCENT }}>Overall Progress</div>
                ...
                {rows.map((r, i) => {
                  ...
                  return (
                    <motion.div key={r.id} style={{ flex: 1, height: 5 }}
                      ...
                      animate={{ scaleX: 1, backgroundColor: s === 'complete' ? ACCENT : s === 'in-progress' ? ACCENT_SOFT : 'rgba(17,24,39,0.07)' }}
              ...
              <div className="flex gap-5">
                {[
                  { label: 'Complete',    color: ACCENT },
                  { label: 'In Progress', color: ACCENT_SOFT },
                  { label: 'Pending',     color: 'rgba(17,24,39,0.12)' },
                ].map(({ label, color }) => (
          ...
          {total > 0 && (
            <motion.div variants={fade}
              ...
              <div className="px-6 sm:px-7 py-4 flex items-center justify-between" style={{ borderBottom: `1px solid ${BORDER}` }}>
                <div className="text-[9px] uppercase tracking-[0.44em] font-bold" style={{ color: ACCENT }}>Milestones</div>
                <AnimatePresence>
                  {liveFlash && (
              ...
              <div className="px-6 sm:px-7 py-7">
                {rows.map((row, i) => {
                  ...
                  const isDone = status === 'complete' || status === 'in-progress';
                  ...
                  return (
                    <motion.div key={row.id} className="flex gap-5 sm:gap-6"
                      ...
                      <div className="flex flex-col items-center shrink-0" style={{ width: 28 }}>
                        ...
                        <div style={{ width: 28, height: 28, flexShrink: 0, marginTop: 2 }}>
                          {status === 'complete' ? (
                            ...
                            <motion.div
                              ...
                              style={{ backgroundColor: WHITE, border: `2.5px solid ${ACCENT}`, outline: `3px solid rgba(0,0,0,0.12)`, color: ACCENT, fontFamily: SERIF }}
                            ...
                            <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-semibold"
                              style={{ backgroundColor: WHITE, border: `1.5px solid ${BORDER}`, color: 'rgba(17,24,39,0.2)', fontFamily: SERIF }}>
                              {num}
                            </div>
                          )}
                        </div>
                        {/* Connector */}
                        {!isLast && (
                          <div style={{ width: 2, flex: 1, minHeight: 24, marginTop: 3, backgroundColor: isDone ? ACCENT : BORDER }} />
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0" style={{ paddingBottom: isLast ? 0 : 28 }}>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          <span className="text-[13px] font-bold" style={{
                            color: dimmed ? 'rgba(17,24,39,0.25)' : DARK,
                            fontFamily: SERIF,
                          }}>
                            {label}
                          </span>
                        ...
                        {desc && (
                          <div className="text-[11px] font-light leading-relaxed mb-2"
                            style={{ color: dimmed ? 'rgba(17,24,39,0.2)' : MUTED }}>
                            {desc}
                          </div>
                        )}
                        ...
                        <div className="text-[9px] uppercase tracking-[0.16em] font-semibold"
                          style={{ color: dimmed ? 'rgba(17,24,39,0.15)' : status === 'in-progress' ? ACCENT : status === 'complete' ? GREEN : MUTED }}>
                          {status === 'pending'
