...
import {
  ...
  Building2, Phone, Mail, GitBranch,
} from 'lucide-react';
      ...
      <motion.div variants={stagger} initial="hidden" animate="show">
        ...
        <motion.div variants={fade} style={{ backgroundColor: WHITE, position: 'relative', overflow: 'hidden', borderBottom: `1px solid ${BORDER}` }}>
          ...
          <div style={{
            ...
            backgroundImage: 'radial-gradient(rgba(157,126,63,0.07) 1px, transparent 1px)',
            backgroundSize: '28px 28px',
          }} />
          {/* 2px gold top accent */}
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, backgroundColor: GOLD }} />

          <div className="px-5 sm:px-8 md:px-10 pt-8 pb-0">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                {/* Eyebrow */}
                <div className="text-[7px] uppercase tracking-[0.48em] font-bold mb-3" style={{ color: GOLD }}>
                  {getGreeting()} · {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                </div>
                {/* Name */}
                <div style={{
                  fontFamily: SERIF, fontStyle: 'italic', fontWeight: 300,
                  fontSize: 'clamp(36px, 7vw, 58px)', color: DARK,
                  lineHeight: 1.03, letterSpacing: '-0.015em',
                }}>
                  {client.name.split(' ')[0]}.
                </div>
                {/* Phase badges */}
                <div className="flex flex-wrap gap-2 mt-4">
                  <span className="text-[7px] uppercase tracking-[0.26em] font-bold px-3 py-1.5"
                    style={{ backgroundColor: 'rgba(157,126,63,0.1)', color: GOLD, border: '1px solid rgba(157,126,63,0.25)' }}>
                    {STATUS_STEPS[stepIdx]?.label}
                  </span>
                  <span className="text-[7px] uppercase tracking-[0.22em] font-semibold px-3 py-1.5"
                    style={{ backgroundColor: 'rgba(26,20,16,0.04)', color: MUTED, border: '1px solid rgba(26,20,16,0.08)' }}>
                    Phase {stepIdx + 1} of {STATUS_STEPS.length}
                  </span>
                </div>
              </div>
              ...
              <div className="hidden sm:block shrink-0 pt-2">
                <ProgressRing pct={ringPct} size={86} />
              </div>
            </div>

            {/* Mini stats strip */}
            <div className="flex items-center gap-0 mt-6"
              style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 16, paddingBottom: 18 }}>
              {[
                { n: messages.length,    label: 'Messages',  color: '#3b82f6' },
                { n: docs.length,        label: 'Documents', color: pendingDocs > 0 ? '#f59e0b' : '#10b981' },
                { n: meetings.length,    label: 'Meetings',  color: '#8b5cf6' },
              ].map(({ n, label, color }, i) => (
                <div key={label} className="flex items-center">
                  {i > 0 && <div className="w-px h-7 mx-4 sm:mx-5" style={{ backgroundColor: BORDER }} />}
                  <div>
                    <div style={{ fontFamily: SERIF, fontSize: 'clamp(20px, 4vw, 26px)', fontWeight: 700, color, lineHeight: 1 }}>{n}</div>
                    <div style={{ fontSize: 7, textTransform: 'uppercase', letterSpacing: '0.24em', color: MUTED, fontWeight: 600, marginTop: 3 }}>
                      {label}
                    </div>
                  </div>
                </div>
              ))}
              {/* Ring on mobile (inside stats strip) */}
              <div className="sm:hidden ml-auto shrink-0">
                <ProgressRing pct={ringPct} size={62} />
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <div className="px-4 sm:px-6 md:px-10 py-6 sm:py-8">
            ...
            {unreadCount > 0 && (
              <motion.div
                ...
                <Link to="/portal/messages"
                  ...
                  style={{ backgroundColor: 'rgba(157,126,63,0.08)', border: '1px solid rgba(157,126,63,0.26)' }}>
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 flex items-center justify-center shrink-0 relative"
                      style={{ backgroundColor: 'rgba(157,126,63,0.15)', border: '1px solid rgba(157,126,63,0.28)' }}>
                    ...
                    <div>
                      ...
                      <div className="text-[10px] font-light" style={{ color: MUTED }}>Tap to view</div>
                    </div>
                  </div>
          ...
          {activeMilestone && (
            <motion.div variants={fade} className="mb-5">
              <Link to="/portal/milestones"
                className="flex items-center justify-between gap-3 px-4 sm:px-5 py-3.5 sm:py-4 group transition-all hover:shadow-sm"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-[7px] uppercase tracking-[0.28em] font-bold mb-0.5" style={{ color: GOLD }}>Current Milestone</div>
                    <div className="text-[12px] font-semibold" style={{ color: DARK }}>{activeMilestone}</div>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 text-[7px] uppercase tracking-[0.2em] font-bold shrink-0"
                  style={{ color: GOLDF }}>
                  View Timeline <ChevronRight className="w-3 h-3" strokeWidth={2} />
                </div>
              </Link>
            </motion.div>
          )}
          ...
          <motion.div variants={fade} className="mb-6 sm:mb-8">
            <div className="text-[7px] uppercase tracking-[0.38em] font-bold mb-3" style={{ color: GOLD }}>
              Quick Access
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-3">
              {QUICK_ACTIONS.map(({ to, Icon, label, sub, accent }) => (
                <Link key={to + label} to={to}
                  className="group p-4 sm:p-5 flex flex-col gap-3 transition-all duration-200 active:scale-[0.97]"
                  style={{
                    backgroundColor: accent ? 'rgba(157,126,63,0.07)' : WHITE,
                    border: `1px solid ${accent ? 'rgba(157,126,63,0.26)' : BORDER}`,
                    boxShadow: '0 1px 4px rgba(26,20,16,0.04)',
                  }}
                  onMouseEnter={e => {
                    const el = e.currentTarget as HTMLElement;
                    el.style.transform = 'translateY(-2px)';
                    el.style.boxShadow = '0 8px 28px rgba(26,20,16,0.1)';
                    el.style.borderColor = accent ? GOLD : 'rgba(157,126,63,0.3)';
                  }}
                  onMouseLeave={e => {
                    const el = e.currentTarget as HTMLElement;
                    el.style.transform = 'none';
                    el.style.boxShadow = '0 1px 4px rgba(26,20,16,0.04)';
                    el.style.borderColor = accent ? 'rgba(157,126,63,0.26)' : BORDER;
                  }}>
                  <div className="flex items-start justify-between">
                    <div className="w-8 h-8 flex items-center justify-center"
                      style={{ backgroundColor: accent ? 'rgba(157,126,63,0.14)' : 'rgba(26,20,16,0.04)' }}>
                      <Icon className="w-3.5 h-3.5" style={{ color: accent ? GOLD : MUTED }} strokeWidth={1.5} />
                    </div>
                    <ChevronRight className="w-3.5 h-3.5 opacity-0 group-hover:opacity-60 transition-opacity"
                      style={{ color: DARK }} strokeWidth={2} />
                  </div>
                  <div>
                    <div className="text-[12px] font-bold leading-tight mb-0.5" style={{ color: DARK, fontFamily: SERIF }}>
                      {label}
                    </div>
                    <div className="text-[9px] font-light leading-relaxed" style={{ color: MUTED }}>{sub}</div>
                  </div>
                </Link>
              ))}
            </div>
          ...
          {/* ── Two-column: Timeline + Builder ── */}
          <motion.div variants={fade} className="grid md:grid-cols-3 gap-4 sm:gap-5 mb-6 sm:mb-8">

            {/* Timeline */}
            <div className="md:col-span-2"
              style={{ backgroundColor: WHITE, border: `1px solid ${BORDER}`, boxShadow: '0 1px 6px rgba(26,20,16,0.04)' }}>
              <div className="px-5 sm:px-7 py-5" style={{ borderBottom: `1px solid ${BORDER}` }}>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-[7px] uppercase tracking-[0.38em] font-bold mb-1" style={{ color: GOLD }}>
                      Project Journey
                    </div>
                    <div className="text-[11px] font-light" style={{ color: MUTED }}>
                      From first brief to handover
                    </div>
                  </div>
                  <span className="text-[7px] uppercase tracking-[0.18em] font-bold px-2.5 py-1.5 shrink-0"
                    style={{ backgroundColor: 'rgba(157,126,63,0.08)', color: GOLD }}>
                    {stepIdx + 1} / {STATUS_STEPS.length}
                  </span>
                </div>
                {/* Progress bar */}
                <div className="mt-4 h-0.5 overflow-hidden" style={{ backgroundColor: 'rgba(26,20,16,0.07)' }}>
                        ...
                        <div key={step.key} className="flex items-start gap-4 relative">
                          ...
                          <div className="flex-1 pb-1">
                            <div className="flex flex-wrap items-center gap-2"
                              style={{ color: done || current ? DARK : 'rgba(26,20,16,0.22)' }}>
                            ...
                            <div className="text-[9px] sm:text-[10px] font-light mt-0.5"
                              style={{ color: done || current ? MUTED : 'rgba(26,20,16,0.15)' }}>
                              {step.detail}
                            </div>
                          </div>
                        </div>
                ...
                {!brief && (
                  <div className="mt-5 pt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
                    ...
                    <Link to="/portal/project"
                      className="inline-flex items-center justify-center gap-2 text-[9px] uppercase tracking-[0.22em] font-black px-5 py-2.5 shrink-0 transition-opacity hover:opacity-85 active:opacity-75"
            ...
            {/* Builder card */}
            <div style={{ backgroundColor: SBG, border: '1px solid rgba(157,126,63,0.12)' }}>
              <div className="p-5 sm:p-6">
                <div className="flex items-center justify-between mb-5">
                  <div className="text-[7px] uppercase tracking-[0.38em] font-bold" style={{ color: GOLD }}>
                    Your Consultant
                  </div>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[7px] font-semibold uppercase tracking-[0.2em]"
                      style={{ color: 'rgba(255,255,255,0.22)' }}>Available</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-12 h-12 flex items-center justify-center text-[14px] font-black shrink-0"
                    style={{
                      background: 'linear-gradient(135deg, rgba(157,126,63,0.28) 0%, rgba(196,167,107,0.1) 100%)',
                      border: '1.5px solid rgba(157,126,63,0.35)',
                      color: GOLDF, fontFamily: SERIF,
                    }}>
                    {BUILDER.initials}
                  </div>
                  <div className="min-w-0">
                    <div className="text-[14px] font-bold" style={{ color: '#FAF7F2', fontFamily: SERIF }}>
                      {BUILDER.name}
                    </div>
                    <div className="text-[9px] font-light mt-0.5" style={{ color: 'rgba(255,255,255,0.3)' }}>
                      {BUILDER.title}
                    </div>
                  </div>
                </div>
                ...
                <p className="text-[10px] sm:text-[11px] leading-relaxed font-light mb-5"
                ...
                <div className="flex flex-col gap-2.5">
                  <a href={`tel:${BUILDER.phone}`}
                    className="flex items-center gap-2.5 text-[10px] font-light min-h-[40px] transition-all active:opacity-60"
              ...
              <div className="px-5 sm:px-6 pb-5 sm:pb-6 flex flex-col gap-2"
                style={{ borderTop: '1px solid rgba(157,126,63,0.1)', paddingTop: 16 }}>
                <Link to="/portal/messages"
                  className="w-full flex items-center justify-center gap-2 text-[9px] uppercase tracking-[0.22em] font-black min-h-[48px] transition-opacity hover:opacity-85 active:opacity-70"
                ...
                <Link to="/portal/meetings"
                  className="w-full flex items-center justify-center gap-2 text-[9px] uppercase tracking-[0.2em] font-bold min-h-[44px] transition-all active:opacity-70"
                  style={{ border: '1px solid rgba(157,126,63,0.3)', color: GOLDF }}
                  onMouseEnter={e => {
                    (e.currentTarget as HTMLElement).style.borderColor = GOLD;
                    (e.currentTarget as HTMLElement).style.backgroundColor = 'rgba(157,126,63,0.1)';
                  }}
                  onMouseLeave={e => {
                    (e.currentTarget as HTMLElement).style.borderColor = 'rgba(157,126,63,0.3)';
          ...
          {recentMsgs.length > 0 && (
            <motion.div variants={fade} className="mb-6 sm:mb-8">
              <div className="flex items-center justify-between mb-3">
                ...
                <Link to="/portal/messages"
                  className="flex items-center gap-1 text-[9px] uppercase tracking-[0.2em] font-bold transition-opacity hover:opacity-70"
                  style={{ color: GOLD }}>
                  All <ArrowUpRight className="w-3 h-3" strokeWidth={2} />
                </Link>
              </div>
              <div style={{ backgroundColor: WHITE, border: `1px solid ${BORDER}`, boxShadow: '0 1px 6px rgba(26,20,16,0.04)' }}>
                {recentMsgs.map((m, i) => (
                  <Link key={m.id} to="/portal/messages"
                    ...
                    <div className="w-8 h-8 flex items-center justify-center text-[10px] font-black shrink-0 mt-0.5"
                      style={{
                        backgroundColor: m.sender === 'builder' ? 'rgba(157,126,63,0.1)' : 'rgba(26,20,16,0.05)',
                        color:           m.sender === 'builder' ? GOLD : 'rgba(26,20,16,0.4)',
                        fontFamily:      SERIF,
                      }}>
                      ...
                      <div className="flex items-baseline gap-2 mb-0.5 flex-wrap">
                        ...
                        {m.sender === 'builder' && (
                          <span className="text-[7px] uppercase tracking-[0.18em] font-bold px-1.5 py-0.5"
                            style={{ backgroundColor: 'rgba(157,126,63,0.08)', color: GOLD }}>HOU INC</span>
                        )}
                        <span className="text-[9px] ml-auto" style={{ color: 'rgba(26,20,16,0.26)' }}>
                          {new Date(m.timestamp).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                        </span>
                      </div>
                      <p className="text-[11px] font-light line-clamp-1" style={{ color: MUTED }}>{m.text}</p>
                    </div>
                    <ChevronRight className="w-3.5 h-3.5 shrink-0 mt-1" style={{ color: 'rgba(26,20,16,0.15)' }} strokeWidth={2} />
                  </Link>
                ))}
              </div>
            </motion.div>
          ...
          {docs.length > 0 && (
            <motion.div variants={fade} className="mb-6 sm:mb-8">
              <div className="flex items-center justify-between mb-3">
                <div className="text-[7px] uppercase tracking-[0.38em] font-bold" style={{ color: GOLD }}>Documents</div>
                <Link to="/portal/documents"
                  className="flex items-center gap-1 text-[9px] uppercase tracking-[0.2em] font-bold transition-opacity hover:opacity-70"
                  style={{ color: GOLD }}>
                  Manage <ArrowUpRight className="w-3 h-3" strokeWidth={2} />
                </Link>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5 sm:gap-3">
                {docs.slice(0, 3).map(doc => (
                  <div key={doc.id}
                    ...
                    style={{ backgroundColor: WHITE, border: `1px solid ${BORDER}`, boxShadow: '0 1px 4px rgba(26,20,16,0.04)' }}>
                    <div className="w-9 h-9 flex items-center justify-center shrink-0"
                      style={{
                        backgroundColor: doc.status === 'uploaded' ? 'rgba(16,185,129,0.09)'
                          : doc.status === 'pending' ? 'rgba(245,158,11,0.09)'
                          : 'rgba(26,20,16,0.05)',
                      }}>
                      {doc.status === 'uploaded'
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[8px] uppercase tracking-[0.18em] mt-0.5 font-semibold"
            ...
            <motion.div variants={fade} className="mb-6 sm:mb-8">
              <Link to="/portal/meetings"
                ...
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-[7px] uppercase tracking-[0.26em] font-bold mb-0.5" style={{ color: '#8b5cf6' }}>
                      Upcoming · {nextMeeting.status === 'confirmed' ? 'Confirmed' : 'Requested'}
                    </div>
                    <div className="text-[13px] sm:text-[14px] font-bold truncate" style={{ color: DARK, fontFamily: SERIF }}>
                      {nextMeeting.type}
                    </div>
          ...
          {/* ── Projects shortcut (md:hidden — bottom nav covers primary; this surfaces Projects) ── */}
          <motion.div variants={fade} className="mb-6 md:hidden">
